...
class ServerRenderer {
  ...
  async setupTexture() {
    ...
    // Load and colorize SVG (you'll need to host this or embed it)
    const svgUrl = this.params.svgUrl || 'http://localhost:3000/raptor-svg.svg';
    const response = await fetch(svgUrl);
    const svgText = await response.text();
    
    const colorizedSvg = svgText
      .replaceAll('fill="#ed1c24"', 'fill="#ffffff"')
      .replaceAll('fill="#231f20"', 'fill="#000000"')
    ...
    // Load SVG as image (this requires a local file or data URL)
